...
def build_collect_rollout(policy, env_params, lookback, balance):
    ...
    Returns a function: (rng_key, trainable_vars, non_trainable_vars, init_state,
                          init_obs, start_idx, n_steps) -> rollout_dict
    """
    bar_feats = env_params._bar_feats if hasattr(env_params, '_bar_feats') else None
    spec = env_params.spec

    def _collect(rng_key, trainable_vars, non_trainable_vars,
                 init_state, init_obs, bar_feats, start_idx, n_steps):

        def scan_body(carry, _step_offset):
            state, obs, step_idx, rng = carry

            # Policy forward (stateless)
            outputs, _ = policy.stateless_call(
                trainable_vars, non_trainable_vars,
                obs[jnp.newaxis], training=False,
            )
            logits, p0_params, p1_params, value = outputs
            logits = logits[0]
            ...
            # Env step
            new_obs, new_state, reward, done, _ = env_step(
            ...
            return (final_state, final_obs, next_idx, rng), transition

        indices = jnp.arange(n_steps)
        (final_state, _, _, _), transitions = jax.lax.scan(
            scan_body, (init_state, init_obs, start_idx, rng_key), indices,
        )
        return transitions, final_state

    return jax.jit(_collect, static_argnums=(7,))
